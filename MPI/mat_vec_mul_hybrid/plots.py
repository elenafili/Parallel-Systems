...
for cmap, color, df in zip(my_cmap, rand, [group for _, group in df_mean.groupby('size')]):
    ...
    # time = df['time w/ comms'].values[0]
    # ax.plot(df['tasks'], time / df['time w/ comms'], '.-', color=cmap(color), label=f'Size: {size}')

    # ax.plot(df['tasks'], df['time'], '.-', color=cmap(color), label=f'Size: {size}')
    # time = df['time'].values[0]
    # ax.plot(df['tasks'], time / df['time'], '.-', color=cmap(color), label=f'Size: {size}')
    # print(df)


ax.legend(loc="best")
plt.savefig(f'./output/plot_speedup_1.png', bbox_inches='tight')
plt.close()